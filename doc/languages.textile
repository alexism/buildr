...
The 2.9.0 Scala compiler supports the following options:

|_. Option        |_. Usage |
| @:debug@        | Generates bytecode with debugging information. @debug@ takes the same values as the @-g@ option: none,source,line,vars,notc. It can also received a boolean and if @true@ the 'vars' level is used. You can also override this by setting the environment variable @debug@ to @off@.|
| @:deprecation@  | If true, shows deprecation messages.  False by default. |
...
In version 2.8 or prior, Scala compiler also supports the following options:

|_. Option        |_. Usage |
| @:debug@         | Generates bytecode with debugging information.  In version 2.8, @debug@ accepts only true or false.|
| @:make@         | Make strategy to be used by the compiler (e.g. @:make=>'transitive'@).|
